...
@login_required
def prof_exams_manage_view(request):
    if request.user.role not in ['professeur', 'admin']:
        return redirect('dashboard')
    ...
    return render(request, 'exams/prof_exams_manage.html', {'exams': exams})

@login_required
def admin_entities_manage_view(request):
    if request.user.role != 'admin':
        return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        if action == 'add_matiere':
            ...
            Classe.objects.create(
                ...
                annee_academique=request.POST.get('annee', '2025-2026')
            )
            messages.success(request, "Classe ajoutée.")
        return redirect('admin_entities_manage')

    return render(request, 'core/admin_entities.html', {
        'matieres': matieres,
        'classes': classes,
        'matiere_choices': MATIERE_CHOICES,
        'classe_choices': CLASSE_CHOICES
